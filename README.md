...
<head>
    ...
    <title>GPT 스마트 플래너 (연보라톤 모바일 최적화)</title>
    <!-- Tailwind CSS CDN -->
...
<div class="container">
    ...
    <h1 class="text-2xl font-bold text-center text-violet-600 py-4">Time blocks 💜</h1>
    ...
    <div class="input-group bg-white p-4 rounded-xl">
        <label for="apiKey" class="block text-sm font-medium text-gray-700 mb-2 flex items-center">
            🔑 OpenAI API Key (암호화 입력)
        </label>
        <!-- Focus ring and border color uses violet-500 -->
        <input type="password" id="apiKey" placeholder="여기에 API Key를 입력하세요" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-violet-500 focus:border-violet-500 text-sm">
    </div>
    ...
    <div class="input-group bg-white p-4 rounded-xl">
        <label for="taskInput" class="block text-sm font-medium text-gray-700 mb-2 flex items-center">
            ✍️ 오늘/이번 주 할 일 목록 (구체적으로 작성)
        </label>
        <!-- Focus ring and border color uses violet-500 -->
        <textarea id="taskInput" rows="4" placeholder="예: 오늘 오픽 공부 3시간, 이메일 확인 1시간, 헬스장 가기, 저녁 식사 준비&#10;주간: 프로젝트 기획안 마무리, 책 1권 읽기" class="w-full p-3 border border-gray-300 rounded-lg focus:ring-violet-500 focus:border-violet-500 text-sm"></textarea>
    </div>
    ...
    <div id="achievementDisplay" class="bg-yellow-100 p-3 rounded-xl text-center font-bold text-yellow-800 hidden">
        🔥 최종 성취도: <span id="achievementScore">0%</span> 달성!
    </div>
    ...
    <div id="scheduleOutput" class="space-y-3 p-4 bg-violet-50 rounded-xl min-h-[150px]">
        <h3 class="text-lg font-semibold text-gray-600 text-center">AI 생성 스케줄이 여기에 표시됩니다.</h3>
    </div>
</div>
...
<script>
    ...
    function saveSchedule() {
        if (currentSchedule.length === 0) {
            return showStatus('저장할 스케줄이 없습니다. 먼저 생성해주세요.', true);
        }
        ...
        showStatus('✅ 현재 스케줄과 피드백이 저장되었습니다.');
    ...
    function loadSchedules() {
        ...
        if (history.length === 0) {
            return showStatus('저장된 스케줄이 없습니다.', true);
        }

        // Output header uses violet-700
        scheduleOutput.innerHTML = `<h3 class="text-xl font-bold mb-4 text-violet-700">📂 저장된 스케줄 목록</h3>`;
        
        history.reverse().forEach((item, index) => {
            const dateStr = item.date;
            const modeStr = item.mode === 'daily' ? '일간' : '주간';
            ...
            loadBtn.innerHTML = `<strong>${modeStr} 플래너 (${dateStr})</strong><br><span class="text-sm text-gray-600">최근 피드백: ${item.feedback || '없음'}</span>`;
            ...
            loadBtn.onclick = () => {
                ...
                showStatus(`✅ ${dateStr} 스케줄을 불러왔습니다. 수정 후 저장하거나 새로 생성하세요.`);
            };
    ...
    function renderSchedule(scheduleData) {
        ...
        // Output header uses violet-700
        scheduleOutput.innerHTML = `<h3 class="text-xl font-bold mb-4 text-violet-700">✨ AI 생성 ${currentMode === 'daily' ? '일간' : '주간'} 스케줄</h3>`;
        ...
        currentSchedule.forEach((task) => {
            ...
            feedbackLabel.textContent = '실제 성과/피드백 (예: 30분은 모자랐다, 집중 어려웠다)';
            ...
            feedbackInput.placeholder = '피드백을 작성하세요... (다음 계획에 반영됩니다)';
    ...
    function constructSystemPrompt(mode, history) {
        ...
        if (recentFeedback.length > 0) {
            historyContext = "사용자의 과거 피드백을 반드시 반영하여 다음 스케줄에 '학습' 효과를 적용해야 합니다. 다음은 최근 피드백 요약입니다:\n";
            recentFeedback.forEach(f => {
                historyContext += `- [${f.date} 피드백]: ${f.feedback}\n`;
            });
            historyContext += "\n";
        }

        const basePrompt = `당신은 개인 맞춤형 AI 플래너입니다. 사용자가 요청한 할 일을 기반으로 시간 기반 일정표를 생성합니다.
        
        **핵심 규칙:**
        1. 시간 기반 일정표만 출력하며, 불필요한 설명, 서론, 결론은 **절대** 포함하지 않습니다. 오직 '시간 ~ 시간 활동명' 또는 '요일: 활동명' 형태의 일정만 출력합니다.
        2. 장시간 일정(2시간 이상)에는 10~15분 휴식 시간(예: 10:30 ~ 10:45 😴 휴식)을 자동으로 포함합니다.
        3. 활동에 적절한 이모지(예: 📚 공부, 🏃 운동, 🍽️ 식사, 💻 업무, 🏠 가사, 💤 휴식/수면)를 카테고리별로 통일하여 사용합니다.
        4. ${mode === 'daily' ? '오늘의 시간별 일정표를 **상세히** 작성합니다. (하루 중 8시간 이상의 활동을 채우는 것을 목표로 합니다.)' : '이번 주의 우선순위를 알아서 배열하여 요일별로 할 일을 채웁니다. (월요일부터 일요일까지 요일별로 구분하여 출력)'}
        5. ${historyContext} 과거 피드백을 분석하여 다음 계획의 시간 배분이나 휴식 추가 등에 **자동으로 반영**해야 합니다. (예: '30분은 짧았다' -> 다음 스케줄에 해당 활동 시간을 늘림)

        요청된 할 일 목록과 모드(${mode})에 맞춰 일정을 생성하세요.`;
    ...
    async function generateSchedule() {
        ...
        if (!apiKey) {
            return showStatus('🔑 OpenAI API Key를 먼저 입력해주세요.', true);
        }
        if (!tasks) {
            return showStatus('📝 할 일 목록을 입력해주세요.', true);
        }

        statusMessage.textContent = '🚀 AI 스케줄 생성 중... (잠시만 기다려주세요)';
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                const response = await fetch(OPENAI_API_URL, {
                    ...
                    body: JSON.stringify({
                        ...
                        messages: [
                            ...
                            { role: 'user', content: `나의 할 일 목록: ${tasks}` }
                        ],
                        temperature: 0.7
                    })
                });
                ...
                if (!response.ok) {
                    ...
                    throw new Error(`API 호출 실패: ${response.status} ${errorData.error ? errorData.error.message : response.statusText}`);
                }
                ...
                if (rawText) {
                    ...
                    showStatus('✅ 스케줄이 성공적으로 생성되었습니다. 피드백을 기록하고 저장하세요!');
                } else {
                    showStatus('⚠️ AI가 유효한 스케줄을 생성하지 못했습니다. 할 일 목록을 구체적으로 작성해 보세요.', true);
                }
                ...
                if (attempt === MAX_RETRIES - 1) {
                    console.error(error);
                    showStatus(`❌ 오류 발생: ${error.message}. API Key 또는 네트워크를 확인하세요.`, true);
                }
    ...
    document.getElementById('clearBtn').addEventListener('click', () => {
        ...
        apiKeyInput.value = '';
        taskInput.value = '';
        scheduleOutput.innerHTML = `<h3 class="text-lg font-semibold text-gray-600 text-center">AI 생성 스케줄이 여기에 표시됩니다.</h3>`;
        currentSchedule = [];
        achievementDisplay.classList.add('hidden');
        showStatus('모든 내용이 초기화되었습니다. (저장된 기록은 유지)');
    });
    ...
    window.onload = () => {
        ...
        showStatus('✨ 플래너 준비 완료! Key와 할 일을 입력하세요.');
    };
